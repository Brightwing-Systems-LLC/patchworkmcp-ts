 ...
 *   - @modelcontextprotocol/sdk  → registerFeedbackTool(server)
...
const PATCHWORK_URL =
  process.env.PATCHWORKMCP_URL ?? "https://patchworkmcp.com";
...
export interface FeedbackToolOptions {
  /** Override PATCHWORKMCP_URL for this tool instance. */
  patchworkUrl?: string;
  /** Override PATCHWORKMCP_API_KEY for this tool instance. */
  apiKey?: string;
  /** Override PATCHWORKMCP_SERVER_SLUG for this tool instance. */
  serverSlug?: string;
}
...
export function registerFeedbackTool(
  ...
  server.tool(
    ...
    async (args: Record<string, unknown>) => {
      const message = await sendFeedback(args, options);
...
export async function sendFeedback(
  ...
  const endpoint = `${url}/api/v1/feedback/`;
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (isRetryableStatus(resp.status) && attempt < MAX_RETRIES) {
        console.warn(
          `PatchworkMCP returned ${resp.status}, retrying (${attempt + 1}/${MAX_RETRIES})`,
        );
